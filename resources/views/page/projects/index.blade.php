...
<div class="min-h-screen bg-[#0a0d12] text-gray-100 px-8 py-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <div class="flex items-center gap-3">
            ...
            {{-- Sort --}}
            <button class="flex items-center gap-2 px-4 py-2 bg-[#161b22] border border-[#30363d] text-gray-400 text-sm rounded-lg hover:border-gray-500 transition-colors">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 mb-8" id="projects-grid">
        ...
        <a href="{{ route('project.generator.index') }}"
           class="group border-2 border-dashed border-[#30363d] hover:border-blue-500/60 rounded-2xl p-8 flex flex-col items-center justify-center gap-4 transition-all duration-300 min-h-[260px] hover:bg-blue-500/5">
            <div class="w-14 h-14 rounded-full bg-[#1c2128] group-hover:bg-blue-600 border border-[#30363d] group-hover:border-blue-500 flex items-center justify-center transition-all duration-300 shadow-lg group-hover:shadow-blue-500/30">
        ...
        <div class="project-card bg-[#161b22] border border-[#30363d] hover:border-[#484f58] rounded-2xl p-5 flex flex-col gap-4 transition-all duration-200 hover:shadow-2xl hover:shadow-black/30"
            ...
            <div class="flex items-start justify-between">
                <div class="w-11 h-11 rounded-xl bg-gradient-to-br {{ $grad }} flex items-center justify-center text-white font-bold text-sm shadow-lg">
                    {{ $initials ?: 'PR' }}
                </div>
                {{-- Status badge: default "Ready" --}}
                <span class="inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
            ...
            <div class="flex flex-wrap gap-2">
                <span class="text-xs text-gray-400 bg-[#0d1117] border border-[#30363d] px-3 py-1 rounded-full">Laravel 12</span>
                <span class="text-xs text-gray-400 bg-[#0d1117] border border-[#30363d] px-3 py-1 rounded-full">{{ $dbLabel }}</span>
                @if($project->db_type === 'pgsql')
                    <span class="text-xs text-gray-400 bg-[#0d1117] border border-[#30363d] px-3 py-1 rounded-full">Livewire</span>
                @endif
            </div>
    ...
    <div class="grid grid-cols-3 gap-5">
        ...
        <div class="bg-[#161b22] border border-[#30363d] rounded-2xl px-6 py-5 flex items-center gap-4">
            ...
            <div>
                ...
                @php $lines = $projects->count() * 3800; $linesStr = $lines >= 1000 ? round($lines/1000, 1).'k' : $lines; @endphp
