<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('project.generator.index') }}" class="text-sm text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300 flex items-center gap-1 transition-colors">
    ...
    <div class="py-6 h-[calc(100vh-100px)]">
        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8 h-full">
            <div class="bg-white dark:bg-gray-800 shadow-xl border border-gray-200 dark:border-gray-700 sm:rounded-xl overflow-hidden h-full flex flex-col md:flex-row shadow-[0_8px_30px_rgb(0,0,0,0.12)]">
                
                <!-- LEFT PANE: Directory Tree -->
                <div class="w-full md:w-1/4 lg:w-1/5 bg-gray-50 dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 flex flex-col h-full overflow-hidden shrink-0">
                    <div class="p-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800/50 flex justify-between items-center">
                        <span class="text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">Explorer</span>
                    ...
                    <div class="flex-1 overflow-y-auto p-2" id="file-tree-container">
                        ...
                        <ul id="file-tree-root" class="text-sm font-mono text-gray-700 dark:text-gray-300 space-y-1"></ul>
                    </div>
                </div>
                ...
                <div class="w-full md:flex-1 flex flex-col h-full bg-[#1e1e1e] border-r border-[#333]">
                    ...
                    <div class="bg-[#2d2d2d] flex items-center border-b border-[#1e1e1e] overflow-x-auto justify-between pr-4">
                        ...
                        <div id="diff-controls" class="hidden flex gap-2">
                            <button id="accept-diff" class="text-xs bg-emerald-600 hover:bg-emerald-500 text-white px-3 py-1 rounded">Accept Changes</button>
                            <button id="reject-diff" class="text-xs bg-[#444] hover:bg-[#555] text-white px-3 py-1 rounded">Reject</button>
                        </div>
                    </div>
                    ...
                    <div class="flex-1 relative overflow-hidden flex" id="editor-container">
                        ...
                        <div class="w-1/2 h-full bg-[#1e1e1e] border-l border-[#444] hidden relative" id="diff-view-pane">
                            <div class="absolute top-0 right-0 p-1 text-xs text-yellow-500 bg-[#333] rounded-bl">Gemini Suggestion</div>
                        ...
                        <div id="code-loading" class="absolute inset-0 bg-[#1e1e1e]/80 backdrop-blur-sm hidden items-center justify-center flex-col z-10 transition-all duration-300 opacity-0 pointer-events-none">
                ...
                <div class="w-full md:w-1/4 lg:w-[300px] flex flex-col h-full bg-white dark:bg-gray-900 shrink-0">
                    <div class="p-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800/50 flex items-center gap-2">
                        <svg class="w-5 h-5 text-purple-500" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm-1-12h2v6h-2zm0 8h2v2h-2z"/></svg>
                    ...
                    <div class="flex-1 overflow-y-auto p-4 flex flex-col gap-4" id="chat-history">
                        <!-- Welcome MSG -->
                        <div class="bg-gray-100 dark:bg-gray-800 p-3 rounded-lg text-sm text-gray-700 dark:text-gray-300 shadow-sm border border-gray-200 dark:border-gray-700">
                            Hi! I'm your AI Engineer. Select a file on the left, then tell me what you want to change in it.
                        </div>
                    </div>

                    <div class="p-3 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                        <textarea id="chat-input" rows="3" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-purple-500 focus:ring-purple-500 text-sm p-3 resize-none" placeholder="e.g. Turn this layout into a grid with 3 columns..."></textarea>
                        <button id="chat-send" class="mt-2 w-full bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-medium py-2 px-4 rounded-lg flex items-center justify-center gap-2 transition-all">
