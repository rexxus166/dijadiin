...
@section('full-layout')
    <!-- Main Layout -->
    <main class="flex-grow flex items-center justify-center relative min-h-[100dvh]">
        ...
        <div
            class="container max-w-[1100px] mx-auto z-10 grid grid-cols-1 lg:grid-cols-12 gap-6 items-center px-4 py-6 md:p-8">
            <!-- Left Column: Value Proposition -->
            <div class="lg:col-span-6 xl:col-span-5 flex flex-col justify-center space-y-5 lg:pr-4 py-2">
                <div class="space-y-3">
                    <h1 class="text-3xl md:text-4xl font-black tracking-tight text-slate-900 dark:text-white leading-[1.1]">
                        Build Laravel Apps <span class="text-primary">Faster with AI</span>
                    </h1>
                    <p class="text-base text-slate-600 dark:text-slate-400 font-medium max-w-md">
                ...
                <div class="grid gap-2.5">
                    <!-- Feature 1 -->
                    <div
                        class="group flex items-center gap-3 p-2.5 rounded-xl border border-transparent hover:border-slate-200 dark:hover:border-slate-800 bg-transparent hover:bg-slate-50 dark:hover:bg-[#1c1f27]/50 transition-all duration-300">
                        <div
                            class="flex-shrink-0 size-8 rounded flex items-center justify-center text-primary/70 group-hover:bg-primary/10 group-hover:text-primary transition-colors">
                            <span class="material-symbols-outlined text-[18px]">code</span>
                        </div>
                        <div>
                            <h3 class="text-sm font-bold text-slate-900 dark:text-white leading-tight">Full Code Ownership
                            </h3>
                            <p class="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Download source code and deploy
                                anywhere.</p>
                        </div>
                    </div>
                    <!-- Feature 2 -->
                    <div
                        ...
                        <div>
                            <h3 class="text-sm font-bold text-slate-900 dark:text-white leading-tight">Laravel 12 Ready</h3>
                            <p class="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Generates clean, modern PHP 8.3
                                code.</p>
                        </div>
                    </div>
                    <!-- Feature 3 -->
                    <div
                        ...
                        <div>
                            <h3 class="text-sm font-bold text-slate-900 dark:text-white leading-tight">No-Code Simplicity
                            </h3>
                            <p class="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Describe your app in plain
                                English.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="pt-2 flex items-center gap-3">
                    <div class="flex -space-x-2">
                        <div
                            class="size-6 rounded-full border border-background-light dark:border-background-dark bg-slate-300 overflow-hidden">
                    ...
                    <p class="text-[11px] font-medium text-slate-500 dark:text-slate-400">Trusted by <span
                            class="text-slate-900 dark:text-white font-bold">1,200+</span></p>
                </div>
            </div>
            <!-- Right Column: Registration Card -->
            <div class="lg:col-span-6 xl:col-span-5 xl:col-start-7 max-w-md mx-auto w-full">
                <div
                    class="bg-white dark:bg-[#1c1f27] rounded-2xl shadow-2xl shadow-slate-900/10 border border-slate-200 dark:border-slate-800 p-6 sm:p-8 flex flex-col justify-center">

                    <!-- Back Button -->
                    <a href="/"
                        class="inline-flex w-fit items-center gap-1.5 text-xs font-medium text-slate-500 hover:text-primary dark:text-slate-400 dark:hover:text-primary transition-colors mb-4">
                        <span class="material-symbols-outlined text-[16px]">arrow_back</span>
                        {{ __('Back to home') }}
                    </a>

                    <div class="mb-5 border-b border-slate-100 dark:border-slate-800/60 pb-4">
                        <h2 class="text-xl font-bold text-slate-900 dark:text-white mb-1">{{ __('Create account') }}</h2>
                        <p class="text-xs text-slate-500 dark:text-slate-400">Start your free trial. No credit card
                            required.</p>
                    </div>

                    <form method="POST" action="{{ route('register') }}" class="space-y-3.5" x-data="{ loading: false }"
                        @submit="loading = true">
                        @csrf
                        ...
                        <div class="space-y-1">
                            <label for="name"
                                class="text-xs font-semibold text-slate-700 dark:text-slate-300">{{ __('Full Name') }}</label>
                            <div class="relative">
                                <div
                        ...
                        <div class="space-y-1">
                            <label for="email"
                                class="text-xs font-semibold text-slate-700 dark:text-slate-300">{{ __('Email Address') }}</label>
                            <div class="relative">
                                <div
                                    class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-400">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                            ...
                            <div class="space-y-1">
                                <label for="password"
                                    class="text-xs font-semibold text-slate-700 dark:text-slate-300">{{ __('Password') }}</label>
                            ...
                            <div class="space-y-1">
                                <label for="password_confirmation"
                                    class="text-xs font-semibold text-slate-700 dark:text-slate-300">{{ __('Confirm') }}</label>
                                <div class="relative">
                                    <div
                                        class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-400">
                        ...
                        <div class="flex items-start gap-2 pt-1">
                            <div class="flex items-center h-4 mt-0.5">
                                <input id="terms" name="terms" required type="checkbox"
                                    class="w-3.5 h-3.5 text-primary bg-slate-50 dark:bg-[#111318] border-slate-300 dark:border-[#3b4354] rounded focus:ring-primary focus:ring-offset-0" />
                            </div>
                            <label for="terms" class="text-[11px] text-slate-500 dark:text-slate-400 leading-tight">
                                I agree to the <a href="#" class="text-primary hover:underline font-medium">Terms</a> and <a
                                    href="#" class="text-primary hover:underline font-medium">Privacy Policy</a>.
                            </label>
                        </div>

                        <!-- Submit Button -->
                        <button type="submit" :disabled="loading" :class="{ 'opacity-80 cursor-wait': loading }"
                            ...
                            <span x-show="!loading" class="text-sm">{{ __('Create Account') }}</span>
                            <span x-show="!loading" class="material-symbols-outlined text-[16px]">arrow_forward</span>
                            <span x-show="loading" style="display: none;" class="text-sm">Creating Account...</span>
                        </button>
                    </form>
